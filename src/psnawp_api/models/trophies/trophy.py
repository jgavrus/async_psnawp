...
from psnawp_api.utils.misc import iso_format_to_datetime, async_zip
...
class TrophyBuilder:
    ...
    def game_trophies(self, platform: Literal["PS Vita", "PS3", "PS4", "PS5"], trophy_group_id: str,
                      limit: Optional[int]) -> AsyncIterator[Trophy]:
    ...
    async def earned_game_trophies_with_metadata(self, account_id: str,
                                                 platform: Literal["PS Vita", "PS3", "PS4", "PS5"],
                                                 trophy_group_id: str, limit: Optional[int]) -> AsyncIterator[Trophy]:
        """Retrieves the earned status with metadata of individual trophy detail of a single - or
        all - trophy groups for a title.
        ...
        trophy_metadata = _get_trophy_from_endpoint(
            API_PATH["trophies_for_title"].format(np_communication_id=self.np_communication_id,
                                                  trophy_group_id=trophy_group_id, ),
            self._request_builder, platform, limit)

        trophy_earned_status = _get_trophy_from_endpoint(
            API_PATH["trophies_earned_for_title"].format(account_id=account_id,
                                                         np_communication_id=self.np_communication_id,
                                                         trophy_group_id=trophy_group_id),
            self._request_builder, platform, limit)

        async for combined_data in async_zip(trophy_metadata, trophy_earned_status):
            combined_data_dict = {}
