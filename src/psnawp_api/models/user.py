...
from psnawp_api.core.psnawp_exceptions import PSNAWPNotFound, PSNAWPForbidden, PSNAWPBadRequest
...
class User:
    ...
    @classmethod
    @create_session
    async def from_online_id(cls, request_builder: RequestBuilder, online_id: str,
                             session: ClientSession = None) -> User:
    ...
    @classmethod
    @create_session
    async def from_account_id(cls, request_builder: RequestBuilder, account_id: str,
                              session: ClientSession = None) -> User:
        ...
        try:
            response = await request_builder.get(
                url=f"{BASE_PATH['profile_uri']}{API_PATH['profiles'].format(account_id=account_id)}", session=session)
            response = await response.json()
    ...
    def __init__(self, request_builder: RequestBuilder, online_id: str, account_id: str):
    ...
    @create_session
    async def profile(self, session: ClientSession = None) -> dict[str, Any]:
        ...
        response = await self._request_builder.get(
            url=f"{BASE_PATH['profile_uri']}{API_PATH['profiles'].format(account_id=self.account_id)}", session=session)

        return await response.json()

    @create_session
    async def get_presence(self, session: ClientSession = None) -> dict[str, Any]:
        ...
        :raises: ``PSNAWPForbidden`` When the user's profile is private,
        and you don't have permission to view their online status.
        ...
        try:
            params = {"type": "primary"}
            response = await self._request_builder.get(
                url=f"{BASE_PATH['profile_uri']}/{self.account_id}{API_PATH['basic_presences']}", params=params,
                session=session)
            return await response.json()
    ...
    @create_session
    async def friendship(self, session: ClientSession = None) -> dict[str, Any]:
    ...
    @create_session
    async def is_blocked(self, session: ClientSession = None) -> bool:
    ...
    async def trophy_summary(self) -> TrophySummary:
        ...
        """
        return await TrophySummary.from_endpoint(request_builder=self._request_builder, account_id=self.account_id)

    def trophy_titles(self, limit: Optional[int] = None) -> AsyncIterator[TrophyTitle]:
        ...
        """
        return TrophyTitles(request_builder=self._request_builder, account_id=self.account_id).get_trophy_titles(
            limit=limit)
    ...
    def trophy_titles_for_title(self, title_ids: list[str]) -> AsyncIterator[TrophyTitle]:
        ...
        .. note::
            ...
            or from :py:meth:`psnawp_api.models.search.Search.get_title_id`
        ...
        """
        return TrophyTitles(request_builder=self._request_builder,
                            account_id=self.account_id).get_trophy_summary_for_title(title_ids=title_ids)
    ...
    def trophies(self, np_communication_id: str, platform: Literal["PS Vita", "PS3", "PS4", "PS5"],
        ...
        .. warning::
            ...
            Setting ``include_metadata`` to ``True`` will use twice the amount of rate limit since the API wrapper
            has to obtain metadata from a separate
            endpoint.
    ...
    async def trophy_groups_summary(self, np_communication_id: str, platform: Literal["PS Vita", "PS3", "PS4", "PS5"],
        ...
        :param platform: The platform this title belongs to.
        :type platform: Literal
        :param include_metadata: If True, will fetch results from another endpoint and include metadata for trophy group
    ...
    def title_stats(self, *, limit: Optional[int] = None, offset: int = 0, page_size: int = 200) -> TitleStatsListing:
        ...
        return TitleStatsListing(request_builder=self._request_builder, account_id=self.account_id,
                                 pagination_arguments=pg_args)
